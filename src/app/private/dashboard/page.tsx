...
const Dashboard = async () => {
  ...
  //for the pop up
  function showAlert() {
    alert('Only Admin has the Access to this Route!');
  }

  return (
    <div className="flex min-h-screen bg-gray-50 dark:bg-gray-900 mt-16">
      {/* Sidebar optns */}
      <div className="w-64 bg-gray-800 text-white h-screen p-6 relative">
        <div className="relative">
          <button className="w-full text-left p-4 hover:bg-gray-700 transition-colors duration-300">
          <Link href="/private/settings" className="block px-4 py-2 text-white-800 transition-colors duration-300 rounded-t-lg">Settings</Link>
          </button>
          <button className="w-full text-left p-4 hover:bg-gray-700 transition-colors duration-300">
          <Link href="/private/addlist" className="block px-4 py-2 text-white-800 transition-colors duration-300 rounded-b-lg">Add to List</Link>
          </button>
        </div>
      </div>
